	...
	return (
		<div className="container mx-auto p-6">
			...
			<div className="flex justify-between items-center mb-4">
				...
				<Link href="#" className="text-primary flex items-center no-underline hover:underline">
					...
					{tabs.map((tab) => (
						<button
							...
							className={`px-4 py-2 rounded-full border-none cursor-pointer font-medium transition-all duration-200 ${
								activeTab === tab.id ? "bg-primary text-white" : "bg-gray-100 text-black hover:bg-gray-200"
							}`}
						>
							{tab.label}
						</button>
					...
					<div key={idx} className="bg-gray-50 rounded-lg p-4 shadow-sm">
						...
						<div className="space-y-3">
							{category.items.map((item, itemIdx) => (
								<div key={itemIdx} className={`flex items-center gap-3 transition-opacity duration-300 ${activeTab === "trending" && !item.trending ? "opacity-50" : "opacity-100"}`}>
