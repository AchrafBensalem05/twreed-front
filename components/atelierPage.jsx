	...
	return (
		<div className="min-h-screen bg-white container mx-auto mt-5">
			...
			<div className="lg:hidden fixed bottom-4 right-4 z-50">
				<button onClick={() => setIsSidebarOpen(true)} className="bg-primary text-white px-4 py-2 rounded-full shadow-lg">
					Filtres
				</button>
			</div>
			...
			<div className="flex flex-col lg:flex-row">
				{/* Sidebar */}
				<div className={`${isSidebarOpen ? "fixed inset-0 z-50" : "hidden"} lg:relative lg:block lg:w-64 bg-white border-r`}>
					{/* Mobile Close Button */}
					...
					<div className="p-4">
						...
						<div className="mb-6">
							...
							<div className="space-y-2">
								{[4.0, 4.5].map((rating) => (
									<Label htmlFor="terms" key={rating} className="flex items-center gap-2 roun">
										<Checkbox id="terms" className="peer w-4 h-4 rounded-full border-gray-300 text-primary focus:ring-primary checked:bg-primary checked:border-primary" />
						...
						<div className="mb-6">
							...
							<div className="space-y-3">
								{["Montres au Quartz", "Montres intelligentes", "Montres mécaniques", "Montres électroniques", "Autres montres"].map((category) => (
						...
						<div className="mb-6">
							...
							<input type="number" placeholder="OK" className="w-20 px-2 py-1 border rounded" />
						</div>
					</div>
				</div>
				...
				<div className="flex-1 lg:max-w-[calc(100%-16rem)]">
					...
					<div className="bg-white p-4 border-b">
						<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
							...
							<span className="text-sm text-gray-500">Affichage des détails pour « vos filtres »</span>
                    ...
                    transition-colors
                    ${selectedFilter === filter.id ? "bg-primary/10 text-primary border border-primary" : "text-gray-600 hover:bg-gray-100"}
                  `}
										>
											{filter.label}
											{selectedFilter === filter.id && <ChevronsUpDown className="w-4 h-4 ml-1" />}
										</button>
									))}
								</div>
							</div>
